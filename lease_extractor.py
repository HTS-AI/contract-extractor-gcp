...
Extracts lease-specific fields from lease agreements.
"""
...
class LeaseExtractor:
    """Extracts lease-specific details from lease documents."""
    ...
    def extract(self, document_text: str) -> Dict[str, Any]:
        """
        Extract lease-specific details from document text.
        
        Args:
            document_text: Raw text from lease document
            
        Returns:
            Dictionary containing extracted lease data
        """
    ...
    def _get_system_prompt(self) -> str:
        """Get the system prompt for lease extraction."""
        return """You are an AI Lease Document Extraction Engine.
You MUST extract ALL relevant lease details with high precision and zero hallucinations.

EXTRACTION RULES:
1. If any data is missing or not present → return null.
2. NEVER hallucinate or invent lease clauses.
...
6. CRITICAL: Extract party names (Lessor and Lessee) from the document header, signature section, or any party identification section. Look for full legal names, company names, or individual names."""
    
    def _create_extraction_prompt(self, document_text: str) -> str:
        """Create the extraction prompt for lease documents."""
        return f"""Analyze the following LEASE document and extract ONLY the following specific information:
...
1. Document Type: Set to "LEASE"
2. Party Names: Extract all party names (Lessor/Landlord/Owner and Lessee/Tenant/Renter). Include full legal names, company names, or individual names. Put primary parties in party_1 and party_2, additional parties in the array.
3. Start Date: Extract the lease start date, effective date, or commencement date. Format as YYYY-MM-DD.
4. Due Date: Extract payment due date, rent due date, or any payment deadline. Format as YYYY-MM-DD.
5. Amount: Extract the payment amount (monthly rent, annual rent, total payment, or any monetary value mentioned). IMPORTANT: Do NOT extract percentages (like "10%", "10 percent", "5%") as amounts - only extract actual monetary values with currency. If only percentages are mentioned without actual amounts, leave amount as empty/null.
6. Frequency: Extract payment frequency (e.g., "Monthly", "Quarterly", "Annual", "One-time").
...
- Look for phrases like "Lessor:", "Landlord:", "Owner:", "Lessee:", "Tenant:", "Renter:"
